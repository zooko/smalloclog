...
//XXXX add threadareanum
//XXXX consider instead, inside the lock, passing events to another thread and that thread can write out to the file? That thread could also compress it!!
unsafe impl GlobalAlloc for SmallocLog {
    unsafe fn alloc(&self, layout: core::alloc::Layout) -> *mut u8 {
        ...
        e[i] = b'a';
        i += 1; // alloc
        e[i..i + U].copy_from_slice(&layout.size().to_le_bytes());
        i += U;
        e[i..i + U].copy_from_slice(&layout.align().to_le_bytes());
        i += U;
        ...
        unsafe {
            assert!(
                MY_FILE_OPT.is_some(),
                "It was created by create_MY_FILE above."
            );
    ...
    unsafe fn dealloc(&self, ptr: *mut u8, layout: core::alloc::Layout) {
        ...
        unsafe {
            assert!(
                MY_FILE_OPT.is_some(),
                "MY_FILE is created in alloc(), which must be called before any call to dealloc()."
            );
            MY_FILE_OPT.as_mut().expect("").write_all(&entry).unwrap();
        }
    ...
    unsafe fn realloc(&self, ptr: *mut u8, layout: Layout, new_size: usize) -> *mut u8 {
        ...
        e[i..i + U].copy_from_slice(&layout.align().to_le_bytes());
        i += U;
        e[i..i + U].copy_from_slice(&new_size.to_le_bytes());
        i += U;
        ...
        unsafe {
            assert!(
                MY_FILE_OPT.is_some(),
                "MY_FILE is created in alloc(), which must be called before any call to realloc()."
            );
            MY_FILE_OPT.as_mut().expect("").write_all(&entry).unwrap();
        }
...
pub fn smalloclog_to_human_readable(sml: &[u8]) -> String {
    ...
    assert!(
        sml[i] == b'3',
        "This version of smalloclog can read only version 3 smallocloc files."
    );
    i += 1;
    ...
    while i < sml.len() {
        if sml[i] == b'a' {
            ...
            writeln!(
                res,
                "realloc(0x{:x}, {}, {}, {}) -> 0x{:x}",
                ptr, siz, ali, newsiz, newptr
            )
            .ok();
        } else {
            panic!(
                "Found something unexpected in smalloclog. sml[{}]: {}, first part: {}, remainder: 0x{:?}",
                i,
                sml[i],
                res,
                &sml[i..]
